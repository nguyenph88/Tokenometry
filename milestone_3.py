...
def get_historical_data(product_id, granularity, years=1):
    ...
    Fetches 1 year of data by default, which is enough for the indicators to mature.
    """
    print(f"Fetching historical data for {product_id} to calculate indicators...")
    try:
        client = RESTClient()
        end_time = int(time.time())
        # We need at least LONG_WINDOW periods of data for the indicators to be valid.
        # Fetching a year gives plenty of buffer.
...
def main():
    """
    Main function to run the live analysis.
    Fetches data, calculates indicators, and prints the latest signal.
    """
