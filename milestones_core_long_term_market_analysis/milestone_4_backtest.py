...
from milestone_4 import get_historical_data, calculate_indicators, generate_signals
from milestone_4 import PRODUCT_ID, GRANULARITY, SHORT_WINDOW, LONG_WINDOW, RSI_PERIOD, MACD_FAST, MACD_SLOW, MACD_SIGNAL, ATR_PERIOD, ATR_STOP_LOSS_MULTIPLIER, RISK_PER_TRADE_PERCENTAGE
...
def run_backtest(df, initial_capital):
    """
    Runs a backtest simulation with ATR-based stop-loss and position sizing.
    """
    print("Running backtest simulation with risk management...")
    ...
    for index, row in df.iterrows():
        # --- Handle Active Trade Management ---
        if active_trade:
            # Check for stop-loss hit
            if row['Low'] <= stop_loss_price:
                exit_price = stop_loss_price # Assume stop-loss executes at the set price
        ...
        # --- Handle New Trade Entry ---
            ...
            # Define stop-loss for this new trade
        ...
        # Update portfolio value for next trade's risk calculation
        if not active_trade:
...
def analyze_performance(df, trades, initial_capital):
    ...
    """
    print("\n--- Backtest Performance Analysis (Full Risk Management) ---")
    ...
    if len(buy_trades) > 0 and len(sell_trades) > 0:
        ...
        # Ensure we have a pair for each trade
...
def main():
    ...
    """
    print("--- Crypto Analysis Bot: Backtesting Milestone 4 ---")
