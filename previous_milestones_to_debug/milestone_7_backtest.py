# milestone_7_backtest.py

import pandas as pd
import matplotlib.pyplot as plt
import matplotlib.dates as mdates

# Import the core logic functions and configurations from the main milestone file
# This ensures the backtest uses the exact same parameters as the live bot.
from milestone_7 import (get_historical_data, calculate_indicators, 
...
def prepare_mta_data(product_id, years=3):
    ...
    This function prepares the complete dataset needed for the simulation.
    """
    print(f"Preparing MTA data for {product_id}...")
    # Fetch 3 years of daily data for signal generation
    df_daily = get_historical_data(product_id, GRANULARITY_SIGNAL, years=years)
    # Fetch 4 years of weekly data to ensure the long-term SMA is mature
    df_weekly = get_historical_data(product_id, GRANULARITY_TREND, years=years+1) 
    ...
    df_weekly['Trend'] = 'Bearish' # Default to Bearish
    # Set Trend to Bullish where the close is above the long-term SMA
    df_weekly.loc[df_weekly['Close'] > df_weekly[trend_sma_col], 'Trend'] = 'Bullish'
    
    # --- Merge Trend Data into Daily Data ---
    # This gives each day the prevailing long-term trend context from its corresponding week.
...
def run_mta_backtest(df, initial_capital):
    """
    Runs a backtest simulation using the MTA filter and full risk management.
    """
    ...
    for index, row in df.iterrows():
        ...
        if active_trade:
            ...
            # 2. Check for a valid SELL signal (Death Cross + Bearish Trend)
        ...
        # --- New Trade Entry ---
        # Enter on a BUY signal ONLY if the long-term trend is Bullish
        ...
        # Update the portfolio value used for the next trade's risk calculation
        if not active_trade:
...
def analyze_performance(df, trades, initial_capital, product_id):
    ...
    if len(buy_trades) > 0 and len(sell_trades) > 0:
        ...
        for i in range(num_cycles):
            ...
            size = buy_trades[i]['size']
            if sell_price > buy_price:
                wins += 1
                total_profit += (sell_price - buy_price) * size
            else:
                losses += 1
                total_loss += (buy_price - sell_price) * size
...
def plot_results(df, trades, product_id):
    ...
    # Shade the background to visualize the long-term trend
    ...
    ax1.set_title(f'{product_id} MTA Backtest (3-Year Performance)')
