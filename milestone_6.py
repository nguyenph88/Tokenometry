...
# Trend Timeframe (Weekly)
GRANULARITY_TREND = "ONE_WEEK"
TREND_SMA_PERIOD = 30 # A 30-week SMA is a good long-term trend indicator
...
def get_historical_data(product_id, granularity, years=1):
    ...
    print(f"Fetching {granularity} data for {product_id}...")
    try:
        client = RESTClient()
        ...
        print(f"  Estimated total periods: {(end_time - start_time) / granularity_seconds:.0f}")
        print(f"  Estimated API calls needed: {((end_time - start_time) / granularity_seconds / 300):.1f}")
        ...
        print(f"An error occurred fetching data for {product_id}: {e}")
        return None

def get_long_term_trend(product_id):
    """Determines the long-term market trend using a weekly SMA."""
    # We need more historical data for a weekly chart
...
def main():
    """Main function to run the multi-asset analysis with MTA filter."""
    print("--- Crypto Analysis Bot: Multi-Asset Scanner with MTA Filter ---")
    results = []
    
    for product_id in PRODUCT_IDS:
        long_term_trend = get_long_term_trend(product_id)
        print(f"Long-term trend for {product_id}: {long_term_trend}")
        ...
        if data is not None and not data.empty:
            ...
            print(f"Could not process daily data for {product_id}, skipping.")
            
    # --- Print Summary Report ---
    print("\n" + "="*60)
    print(" " * 15 + "MULTI-ASSET SIGNAL SUMMARY (MTA FILTERED)")
    print("="*60)
